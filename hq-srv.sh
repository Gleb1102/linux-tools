...
DNS_FWD_DEF="1.1.1.1 8.8.8.8"
SSH_USER_DEF="sshuser"
SSH_PASS_DEF="P@ssw0rd"
SSH_UID_DEF="1010"
SSH_PORT_DEF="2024"
SSH_TRIES_DEF="2"
...
write_files(){
  ...
  # Создаем env-файл (без экспорта сложных кавычек)
...
if [ -n "$IFACE" ]; then
  # Настройка физического интерфейса (без IP)
...
# 3. Настройка DNS (dnsmasq) - только готовим конфиг
...
cname=moodle.$DNS_DOMAIN,hq-rtr.$DNS_DOMAIN
cname=wiki.$DNS_DOMAIN,hq-rtr.$DNS_DOMAIN
EOT
...
echo "--- Для генерации DNS таблиц (Записи A, PTR, CNAME) ---"
