...
ADMIN_PASS_DEF="P@\$\$word"

# Внешняя сеть
WAN_IP_DEF="172.16.4.2/28"
WAN_GW_DEF="172.16.4.1"
...
# GRE и OSPF
GRE_REMOTE_DEF="172.16.5.2"
GRE_INNER_DEF="172.16.0.1/30"
OSPF_PASS_DEF="P@ssw0rd"
...
V100_HOSTS="$(ask "Локальная сеть в сторону HQ-SRV должна вмещать не более (адресов)" "$V100_HOSTS_DEF")"
...
V200_HOSTS="$(ask "Локальная сеть в сторону HQ-CLI должна вмещать не более (адресов)" "$V200_HOSTS_DEF")"
...
V999_HOSTS="$(ask "Локальная сеть для управления должна вмещать не более (адресов)" "$V999_HOSTS_DEF")"
...
# 5. Привязка DHCP и Subscriber Map к интерфейсу BMI
cmd "interface bmi.$env(V200_ID)"
cmd "dhcp-server 1"
cmd "subscriber-map MAP_LAN"
cmd "routes-install"
cmd "exit"

# 6. Маршрутизация и NAT
...
# 7. GRE Туннель и OSPF с паролем
...
send_user "\n[УСПЕХ]: Настройка HQ-RTR завершена.\n"
send "\x0f"
EOF

echo "[OK] Настройка успешно завершена."